        ...
        for(int b = 0; b < batch_count; b++)
        {
            ...
            for(int i = 0; i < N; i++)
            {
                ...
                for(int j = 0; j < M; j++)
                {
                    ...
                    // if(hX[b][idx] != 0)
                    // {
                    err_1 += rocblas_abs((hX[b][idx] - hXorB_1[b][idx]));
                    err_2 += rocblas_abs((hX[b][idx] - hXorB_2[b][idx]));
                    err_scal += rocblas_abs(hX[b][idx]);
                    // }
                    // else
                    // {
                    //     err_1 += rocblas_abs(hXorB_1[b][idx]);
                    //     err_2 += rocblas_abs(hXorB_2[b][idx]);
                    // }
                }
            ...
            for(int i = 0; i < N; i++)
            {
                ...
                for(int j = 0; j < M; j++)
                {
                    ...
                    // if(hB[b][j + i * ldb] != 0)
                    // {
                    res_1 += rocblas_abs((hXorB_1[b][idx] - hB[b][idx]));
                    res_2 += rocblas_abs((hXorB_2[b][idx] - hB[b][idx]));
                    // }
                    // else
                    // {
                    //     res_1 += rocblas_abs(hXorB_1[b][idx]);
                    //     res_2 += rocblas_abs(hXorB_2[b][idx]);
                    // }
                    err_scal += rocblas_abs(hB[b][idx]);
                }
                max_err_scal = max_err_scal > err_scal ? max_err_scal : err_scal;
