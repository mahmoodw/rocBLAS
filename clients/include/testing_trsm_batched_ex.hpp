        ...
        for(int b = 0; b < batch_count; b++)
        {
            for(int i = 0; i < N; i++)
            {
                ...
                for(int j = 0; j < M; j++)
                {
                    // if(hX[b][j + i * ldb] != 0)
                    // {
                        err_1 += rocblas_abs((hX[b][j + i * ldb] - hXorB_1[b][j + i * ldb]));
                        err_2 += rocblas_abs((hX[b][j + i * ldb] - hXorB_2[b][j + i * ldb]));
                        err_scal += rocblas_abs(hX[b][j + i * ldb]);
                    // }
                    // else
                    // {
                    //     err_1 += rocblas_abs(hXorB_1[b][j + i * ldb]);
                    //     err_2 += rocblas_abs(hXorB_2[b][j + i * ldb]);
                    // }
                }
            ...
            // const double rel_error = eps * error_eps_multiplier * sqrt(M);
            // near_check_general<T>(M, N, batch_count, ldb, hX, hXorB_1, rel_error);
            // near_check_general<T>(M, N, batch_count, ldb, hX, hXorB_2, rel_error);
            ...
            for(int i = 0; i < N; i++)
            {
                ...
                for(int j = 0; j < M; j++)
                {
                    // if(hB[b][j + i * ldb] != 0)
                    // {
                        res_1 += rocblas_abs((hXorB_1[b][j + i * ldb] - hB[b][j + i * ldb]));
                        res_2 += rocblas_abs((hXorB_2[b][j + i * ldb] - hB[b][j + i * ldb]));
                        err_scal += rocblas_abs(hB[b][j + i * ldb]);
                    // }
                    // else
                    // {
                    //     res_1 += rocblas_abs(hXorB_1[b][j + i * ldb]);
                    //     res_2 += rocblas_abs(hXorB_2[b][j + i * ldb]);
                    // }
                }
                max_err_scal = max_err_scal > err_scal ? max_err_scal : err_scal;
            ...
            // const double res_error = eps * residual_eps_multiplier * M; 
            // near_check_general<T>(M, N, batch_count, ldb, hXorB_1, hB, res_error);
            // near_check_general<T>(M, N, batch_count, ldb, hXorB_2, hB, res_error);
    }
